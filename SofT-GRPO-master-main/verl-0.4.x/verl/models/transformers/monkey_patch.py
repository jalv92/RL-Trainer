...
try:
    from transformers.modeling_flash_attention_utils import _flash_attention_forward as _hf_flash_attention_forward

    _FLASH_ATTENTION_AVAILABLE = True
    _FLASH_ATTENTION_IMPORT_ERROR = None
except Exception as exc:  # pragma: no cover - depends on system packages
    _hf_flash_attention_forward = None
...
def _require_flash_attention():
    if not _FLASH_ATTENTION_AVAILABLE:
        raise RuntimeError(
            "Flash attention backend is unavailable for monkey_patch "
            "operations. Please install flash_attn or switch the "
            "attention implementation to 'eager'. "
            f"Original import error: {_FLASH_ATTENTION_IMPORT_ERROR}"
        )
...
def _ulysses_flash_attention_forward(
    ...
    **kwargs,
    ...
    _flash_attention_forward = _hf_flash_attention_forward
...
def apply_monkey_patch(
    ...
    if not _FLASH_ATTENTION_AVAILABLE:
        logger.warning(
            "Skipping flash attention monkey patch for %s because flash_attn backend is unavailable%s.",
            model.__class__.__name__,
            f" ({_FLASH_ATTENTION_IMPORT_ERROR})" if _FLASH_ATTENTION_IMPORT_ERROR else "",
        )
        return
