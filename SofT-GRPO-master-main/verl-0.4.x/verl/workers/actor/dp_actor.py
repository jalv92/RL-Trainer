...
if is_cuda_available:
    ...
    except Exception as exc:  # pragma: no cover - only hit on machines without flash_attn
        index_first_axis = pad_input = rearrange = unpad_input = None
        _FLASH_ATTN_IMPORT_ERROR = exc

if not FLASH_ATTENTION_SUPPORT and is_npu_available:
...
class DataParallelPPOActor(BasePPOActor):
    def __init__(self, config, actor_module: nn.Module, actor_optimizer: torch.optim.Optimizer = None):
        ...
        if self.use_remove_padding and not FLASH_ATTENTION_SUPPORT:
            logger.warning(
                "Flash attention backend is unavailable%s; disabling remove_padding for actor.",
