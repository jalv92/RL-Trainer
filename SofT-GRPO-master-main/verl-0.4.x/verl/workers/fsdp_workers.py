...
def _flash_attention_available():
    global _FLASH_ATTN_AVAILABLE, _FLASH_ATTN_IMPORT_ERROR
...
def _resolve_attn_implementation(model_cfg, override_cfg=None, component_name="model"):
    override_cfg = override_cfg or {}
    fallback = "eager"
    if model_cfg is not None:
        fallback = model_cfg.get("attn_fallback", fallback)
    attn_impl = override_cfg.get("attn_implementation")
    if attn_impl is None and model_cfg is not None:
        attn_impl = model_cfg.get("attn_implementation")
    attn_impl = attn_impl or "flash_attention_2"
    if attn_impl == "flash_attention_2" and not _flash_attention_available():
        logger.warning(
            "Flash attention backend unavailable%s while initializing %s; using %s attention instead.",
            f" ({_FLASH_ATTN_IMPORT_ERROR})" if _FLASH_ATTN_IMPORT_ERROR else "",
            component_name,
            fallback,
        )
        attn_impl = fallback or "eager"
    return attn_impl
...
class ActorRolloutRefWorker(Worker):
    ...
    def _build_model_optimizer(
        ...
        override_model_config = override_model_config or {}
        attn_impl = _resolve_attn_implementation(self.config.model, override_model_config, component_name=f"{role} model")
        ...
        override_config_kwargs["attn_implementation"] = attn_impl
...
class CriticWorker(Worker):
    ...
    def _build_critic_model_optimizer(self, config):
        ...
        attn_impl = _resolve_attn_implementation(config.model, override_config, component_name="critic model")
        override_config_kwargs["attn_implementation"] = attn_impl
        if self.rank == 0:
...
class RewardModelWorker(Worker):
    ...
    def _build_model(self, config):
        ...
        trust_remote_code = config.model.get("trust_remote_code", False)
        attn_impl = _resolve_attn_implementation(config.model, None, component_name="reward model")
