...
_SGLANG_SUPPORTS_SOFT_THINKING = "enable_soft_thinking" in inspect.signature(ServerArgs.__init__).parameters
...
class SGLangRollout(BaseRollout):
    ...
    def _init_inference_engine(self, trust_remote_code, actor_module, port):
        ...
        if first_rank_in_node:
            ...
            if _SGLANG_SUPPORTS_SOFT_THINKING:
                engine_kwargs["enable_soft_thinking"] = enable_soft_thinking
            elif enable_soft_thinking:
                logger.warning(
                    "SGLang build missing enable_soft_thinking support; disabling soft-thinking features for rollout."
                )

            server_init_params = inspect.signature(ServerArgs.__init__).parameters
            if "add_noise_dirichlet" in server_init_params:
                engine_kwargs["add_noise_dirichlet"] = self.config.get("add_noise_dirichlet", False)
            elif self.config.get("add_noise_dirichlet", False):
                logger.warning(
                    "SGLang build missing add_noise_dirichlet support; disabling Dirichlet noise injection for rollout."
                )
            if "add_noise_gumbel_softmax" in server_init_params:
                engine_kwargs["add_noise_gumbel_softmax"] = self.config.get("add_noise_gumbel_softmax", False)
            elif self.config.get("add_noise_gumbel_softmax", False):
                logger.warning(
                    "SGLang build missing add_noise_gumbel_softmax support; disabling Gumbel noise injection for rollout."
                )

            # Drop any kwargs that ServerArgs doesn't accept to stay compatible with older builds.
            supported_keys = set(server_init_params.keys())
            pruned_kwargs = {}
            for key, value in engine_kwargs.items():
                if key in supported_keys:
                    pruned_kwargs[key] = value
                else:
                    logger.warning("SGLang ServerArgs missing '%s'; dropping from engine init.", key)

            self._engine = AsyncEngine(**pruned_kwargs)
        else:
            self._engine = None
