...
# Mock LLM for testing CoT without GPU
class MockLLMForCoT:
    """Mock LLM that generates realistic CoT responses for testing."""
    ...
    def _generate_response(self, prompt):
        ...
        if "Analysis:" in prompt:
            return "Market is in a strong downtrend with RSI at 35 (oversold). Price is approaching key support at $20,100 with declining volume suggesting momentum is weakening."
        elif "Options:" in prompt:
            return """Option A: BUY - Potential counter-trend reversal at oversold support
Option B: WAIT - Need confirmation of reversal pattern
Option C: SELL - Trend continuation play if support breaks"""
        elif "Risks:" in prompt:
            return """Option A Risk: Risk of trend continuation lower, could lose $200-300
Option B Risk: Missing early entry, opportunity cost of waiting
Option C Risk: Selling at support may have poor risk/reward ratio"""
        elif "Final Decision:" in prompt:
            return """Decision: WAIT
Confidence: 0.75
Reasoning: The risk/reward is unclear at current levels. While oversold, the strong downtrend suggests waiting for confirmation. A bullish reversal candle at support would improve the setup significantly."""
        else:
            return "HOLD (default)"
...
if __name__ == '__main__':
    ...
    # Create mock LLM
    mock_llm = MockLLMForCoT()
    cot_reasoner = ChainOfThoughtReasoner(mock_llm, cache_size=10)
    ...
    # Test reasoning
    action, confidence, reasoning_chain = cot_reasoner.reason(
        obs, position_state, market_context, available_actions
    )
    ...
    for step, content in reasoning_chain.items():
        print(f"   {step}: {str(content)[:80]}...")

    print(f"\n✅ Total LLM queries: {mock_llm.total_queries} (4 expected for 4 CoT steps)")
